...
import os
import cv2  # Required for the scanner
import properties
import access
...
class App(ctk.CTk):
    ...
    def login(self):    
        ...
        cursor.execute(properties.get_user, {"uname": username, "pwd": password})
...
class v_voter_status:  
    def __init__(self, parent):
        self.parent = parent
        # Clear previous frame if switching
        if hasattr(parent, 'login_frame'):
            parent.login_frame.destroy()
        ...
        self.search_btn = ctk.CTkButton(self.status_frame, text="Search",state ="normal", command=self.user_search)
        self.search_btn.grid(row=2, column=0, pady=15)

        self.reset_btn = ctk.CTkButton(self.status_frame, text="Reset", state ="normal", fg_color="gray", command=self.reset_fields)
        ...
        self.print_btn = ctk.CTkButton(self.status_frame, text="Print", state ="disabled",command=self.printQr)
    ...
    def generate_qr(self):
        ...
        # Call service - Ensure service uses 'voter_id' argument correctly
    ...
    def printQr(self):
        ...
        print_status = qrGeneratorService.qrGeneratorService.printQrService(self,voter_id,file_path)
        
        if print_status == 1:
            # SUCCESS: Reset buttons for the next voter
             messagebox.showinfo("Success", "Printing completed. disabling the search.")
             self.Gqr_btn.configure(state="disabled") # Must search before generating
             self.print_btn.configure(state="disabled") # Cannot print until new QR generated
        
            # Optional: Clear the entry and labels for the next person
             self.voterid_Entry.delete(0, tk.END)
             self.result_label.configure(text="Ready for next scan", text_color="white")
             self.scan_qr_code(self,voter_id)
        else:
        # FAILED: Keep buttons as they were so the user can try again
             self.print_btn.configure(state="normal")


    async def scan_qr_code(self,voter_id) :
        getValue = await scanQr.ScanQr.start_scanner(voter_id)
        if getValue and getValue == 1:
